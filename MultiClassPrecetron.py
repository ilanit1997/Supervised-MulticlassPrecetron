...
class PerceptronClassifier:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        while True:
            ...
            for i in range(m):
                xi = X[i]
                yi = np.argmax(self.w @ xi.T) #predicted
                if yi != y[i]: #comparing true label to predicted label
                    self.w[yi] -= xi #substructing false classification
                    self.w[y[i]] +=xi #adding to true classification
                    is_changed=True
            if is_changed == False:
                break
        return self.w
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in range(m):
            xi = X[i]
            y.append(np.argmax(self.w @ xi.T))
        return np.array(y)
